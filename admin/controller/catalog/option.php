...
class ControllerCatalogOption extends Controller {
	private $error = array();
			...
			} elseif (isset($this->request->post['selected'])) {			$approved = 0;			
			foreach ($this->request->post['selected'] as $option_id) {										
			$option_info = $this->model_catalog_option->getOption($option_id);								
			if ($option_info && !$option_info['approve']) {					$this->model_catalog_option->approveData($option_id);
			$approved++;				}			} 						
			...
			$this->response->redirect($this->url->link('catalog/option', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL'));					}		
			$this->getList();	
			}
	...
	protected function getList() {
		...
		$url = '';
            ...
            /*START OVICKO MULTISELLER*/
				$data['approve'] = $this->url->link('catalog/option/approve', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL');
			/*END OVICKO MULTISELLER*/
