...
public function updateexpirycusts(){
	...
	public function editSeller($data) {
		...
		if (isset($data['productimage'])) {
			...
			image = '". $this->db->escape(html_entity_decode($data['productimage'], ENT_QUOTES, 'UTF-8')) . "' 
			WHERE seller_id = '" . (int)$this->seller->getId() . "'");
		}
	...
	public function addSellerMessage($data = array(),$seller_id)
	{
		$sql = "INSERT INTO " . DB_PREFIX . "seller_message SET 
		seller_id = '" .$this->db->escape($seller_id) . "',
		customer = '" .$this->db->escape($data['name']) . "',
