...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		foreach ($order_products as $order_product) {
			...
			if($order_product['seller_id']) {
						$data['mproducts'][$order_product['seller_id']][] = array(
		...
		$from = $this->model_setting_setting->getSettingValue('config_email', $order_info['store_id']);
			...
			foreach($data['mproducts'] as $seller_id=>$sellerprods){
					$get_seller_info = $this->db->query("SELECT email FROM `" . DB_PREFIX . "sellers` WHERE 
					seller_id = '" . (int)$seller_id. "'");
					$data['mulproducts'] =$sellerprods; 
					$seller_email = $get_seller_info->row['email'];
					$language = new Language($order_info['language_code']);
					$language->load($order_info['language_code']);
					$language->load('mail/morder');
					$subject = sprintf($language->get('text_new_subject'), html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'), $order_info['order_id']);
					$html1 = $this->load->view('mail/morder', $data);
					$mail = new Mail();
